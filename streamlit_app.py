    ...
    with col1:
        ...
        if uploaded_file is not None:
            ...
            with st.expander(translate_text("Banner Setting", "Banner设置")):
                ...
                # 指定图片位置
                banner_width = st.slider(translate_text("Banner Width", "Banner宽度"), 100, 1500, 1500,5)
                banner_height = st.slider(translate_text("Banner Height", "Banner高度"), 100, 500, 500,5)

            with st.expander(translate_text("Image Setting", "图片设置")):
                # 根据banner_size调整position的最大值和最小值
                position_x = st.slider(translate_text("Image Position(X)", "图片位置(X)"), -banner_height, banner_width, 100,5)
                position_y = st.slider(translate_text("Image Position(Y)", "图片位置(Y)"), -banner_height, banner_height, 0,5)
            ...
            with st.expander(translate_text("Text Setting", "文字设置")):
                ...
                # 指定Banner文字位置
                text_position_x = st.slider(translate_text("Text Position(X)", "文字位置(X)"), -banner_width, banner_width, 1000,5)
                text_position_y = st.slider(translate_text("Text Position(Y)", "文字位置(Y)"), -banner_height, banner_height, -200,5)
